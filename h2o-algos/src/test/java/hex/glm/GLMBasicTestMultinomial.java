...
//        Assert.assertEquals(0,model.coefficients().get("Intercept"),0);
  ...
  public void testCODGradients(){
    ...
    try {
      ...
      compareGLMCoeffs(manualCoeff, model._output._submodels[0].beta, 2e-3);  // compare two sets of coeffs
  ...
  public void compareGLMCoeffs(double[] coeff1, double[] coeff2, double tol) {
    ...
    for (int index=0; index < coeff1.length; index++) {
      assertTrue(Math.abs(coeff1[index]-coeff2[index]) < tol);  // make sure coefficients are close
    }
  }
  ...
  public double[] getCODCoeff(Frame train, double alpha, double lambda, double[] ymu, DataInfo tinfo) {
    ...
    double[] gradV = new double[]{0.257139634833002, 0.46690465882579507, 0.39275752583520174, 0.5195161101729449,
            1.022270717339313, 0.3263279837988917, 0.4094723914766154, 0.9580737382159515, 1.1276925415149,
            1.1402071096250586, 0.3289838712927178, -0.26699615603698656, -0.4762064854307763, -0.5416108081882695,
            0.44816354113901957, 0.9594573896369526, 0.6926294757031837, 0.7707998828798805, 1.092504862982794,
            1.3208120079506411, 0.9026500040489451, -0.06799067489706279, 0.544490657715165, 0.8247628871551242,
            1.2048356677712784, 2.015231245515948, 2.5570463493450317, 3.2147471030594996};
